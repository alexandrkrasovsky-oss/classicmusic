...
one('''account=text("выйти",12,activeTheme.accent);account.setTypeface(uiFontStrong());account.setGravity(Gravity.CENTER);account.setPadding(dp(6),dp(7),dp(6),dp(7));account.setOnClickListener(v->logout());root.addView(account,new LinearLayout.LayoutParams(-1,dp(32)));''','''footerMark=new ImageView(this);footerMark.setImageResource(ru.classic.music.R.drawable.ic_tm_mark);footerMark.setScaleType(ImageView.ScaleType.CENTER_INSIDE);footerMark.setAlpha(.12f);LinearLayout.LayoutParams markLp=new LinearLayout.LayoutParams(-1,dp(26));markLp.setMargins(0,dp(2),0,0);root.addView(footerMark,markLp);
        account=text("выйти",12,activeTheme.accent);account.setTypeface(uiFontStrong());account.setGravity(Gravity.CENTER);account.setPadding(dp(6),dp(7),dp(6),dp(7));account.setOnClickListener(v->logout());root.addView(account,new LinearLayout.LayoutParams(-1,dp(32)));''','add footer mark')
